...
class ProductService extends Model
{
    ...
    public static function vatData($vats)
    {
        $vatArr = explode(',', $vats);
        ...
        foreach($vatArr as $vat)
        {
            $vatsData = Vat::find($vat);
            $vats[]   = !empty($vatsData) ? $vatsData->name : '';
        }
...
//            dd($pos);
    ...
    public static function vat_id($product_id)
    {
        $vat = DB::table('product_services')
        ->where('id', $product_id)
        ->where('created_by', Auth::user()->creatorId())
        ->select('vat_id')
        ->first();

        return ($vat != null) ? $vat->vat_id : 0;
    }
