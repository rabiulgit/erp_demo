...
//            )->where('created_by', $user->id)->get()->toArray();
    ...
    function getProjectsapi() {
        ...
        $project = Project::with('tasks')->select(
                           [
                               'project_name',
                               'id',
                               'client_id',
    ...
    public function receiveAttendanceData(Request $request)
    {
        $attendanceDatas = $request->all();
        // Process and save the logs as necessary
        foreach ($attendanceDatas ?? [] as $attendanceData) {

            try {
                $existingLog = AttendanceLog::where('employee_id', $attendanceData['employee_id'])
                    ->where('date', $attendanceData['date'])
                    ->first();

                if (is_null($existingLog)) {

                    $existingLog = AttendanceLog::create($attendanceData);
                } else {
                ...
                Log::error('Error saving attendance Log for Employee_id: ' . $attendanceData->employee_id . "msg" . $e->getMessage());
                continue; 
            }
        }
