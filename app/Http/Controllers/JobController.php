        ...
        if(\Auth::user()->can('manage job'))
        {
            ...
            if ($isApproval->isEmpty()) {
                ...
                if ($order == $minOrder) {
                    ...
                    $allJobs = Job::where('created_by', '=', \Auth::user()->creatorId())->with(['branches','createdBy'])->get();

                    foreach ($allJobs as $job) {
                        $approvalArray=ApprovalStatus::where('module', $module)->where('module_id', $job->id)
            ...
            foreach ($jobs as $job) {
                ...
                if(!$isApproval->isEmpty()){
                    $statusChecked=ApprovalStatus::where('module', $module)
                        ->where('module_id','=',$job->id)
                        ->where('role_id','=',$roleId)->first();
    
                    if(isset($statusChecked)){
                        $job->statusChecked=1;
                        $job->leaveStatus=$statusChecked->status;
                    }
                    else{
                        $job->statusChecked=0;
                    }
            ...
            return view('job.index', compact('jobs', 'data', 'isAdmin'));
        }
        else
        {
...
//            }
    ...
    public function approveJob($id)
    {
        ...
        $totalApproval = Approval::where('module', 'recruitment')->count();

        $totalApproved = ApprovalStatus::where('module', 'recruitment')->where('module_id', $id)->count();

        if($totalApproval == $totalApproved){
            Job::where('id', $id)->update(['approvals' => 'Approved']);
        }

        return redirect()->route('job.index')->with('success', __('Recruitment Approved.'));
    }
    ...
    public function rejectJob($id)
    {
        ...
        Job::where('id', $id)->update(['approvals' => 'Rejected']);

        return redirect()->route('job.index')->with('success', __('Recruitment Rejected.'));
    }
