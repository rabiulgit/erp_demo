    ...
    public function appendTimesheetTaskHTML(Request $request)
    {
        ...
        if($project)
        {
            ...
            if($task && $selected_dates)
            {
                ...
                {
                    $returnHTML .= '<td class="text-center"><span class="task-time" data-ajax-timesheet-popup="true" data-type="create" data-task-id="' . $task->id . '" data-date="' . $dateobj->format('Y-m-d') . '" data-url="' . route('timesheet.create', $project_id) . '"><i class="ti ti-plus me-2 cursor-pointer"></i></span></td>';
                }
