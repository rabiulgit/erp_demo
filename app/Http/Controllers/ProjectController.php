...
class ProjectController extends Controller
{
    ...
    public function show(Project $project)
    {
        ...
        if(\Auth::user()->can('view project'))
        {
            ...
            if(!empty($employee->team_lead)){
                if($employee->team_lead == '1'){
                    if(!empty($employee->team_id)){
                        $teams=Employee::where('team_id',$employee->team_id)->where('id', '!=', $employee->id)->get();
                        $team_projects = [];

                        foreach ($teams as $teamMember) {
                            $userProjects = $teamMember->user->projects;
            
                            foreach ($userProjects as $project) {
                                $team_projects[$project->id] = $project->id;
                            }
                        }

                        $user_projects += $team_projects;
                    }
                    ...
                    if(\Auth::user()->type == 'client'){
                      $bugs = Bug::where('project_id',$project->id)->get();
                        ...
                        if(!empty($employee->team_lead)){
                            if ($employee->team_lead == '1') {
                                if(!empty($employee->team_id)){
                                    $teamMembers = Employee::where('team_id', $employee->team_id)->pluck('user_id')->toArray();
                                    $teamMemberIds = array_merge($teamMemberIds, $teamMembers);
                                }
                            }
                        }
                        ...
                        ->where(function ($query) use ($teamMemberIds) {
                            foreach ($teamMemberIds as $userId) {
                                $query->orWhereRaw("FIND_IN_SET('$userId', assigned_users)");
                            }
                        })
                        ->with(['assignTo', 'createdBy', 'bug_status', 'project'])
                        ->get();
                    }
                }
    ...
    public function bugStore(Request $request, $project_id)
    {
        if(\Auth::user()->can('create bug report'))
        {
            ...
            if (!in_array(Auth::user()->id, $assigned)) {
            ...
            $bug->assigned_users = implode(',', $assigned);
    ...
    public function bugUpdate(Request $request, $project_id, $bug_id)
    {
        ...
        if(\Auth::user()->can('edit bug report'))
        {
            ...
            $assigned = [];
            $assigned = explode(',', $bug->assigned_users);

            if (!in_array($request->assign_to, $assigned)) {
                $assigned[] = $request->assign_to;
            }
