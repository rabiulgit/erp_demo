...
class DealController extends Controller
{
    ...
    public function index()
    {
        ...
        if($usr->can('manage deal'))
        {
            ...
            $deals       = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->where('approval', 'Approved')->get();
            $curr_month  = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->whereMonth('created_at', '=', date('m'))->where('approval', 'Approved')->get();
            $curr_week   = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->where('approval', 'Approved')->whereBetween(
                'created_at', [
                                \Carbon\Carbon::now()->startOfWeek(),
                                \Carbon\Carbon::now()->endOfWeek(),
                            ]
            )->get();
            $last_30days = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->where('approval', 'Approved')->whereDate('created_at', '>', \Carbon\Carbon::now()->subDays(30))->get();
            // Deal Summary
            $cnt_deal                = [];
    ...
    public function deal_list()
    {
        ...
        if($usr->can('manage deal'))
        {
            ...
            $deals       = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->where('approval', 'Approved')->get();
            $curr_month  = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->where('approval', 'Approved')->whereMonth('created_at', '=', date('m'))->get();
            $curr_week   = Deal::whereIn('id', $id_deals)->where('pipeline_id', '=', $pipeline->id)->where('approval', 'Approved')->whereBetween(
                'created_at', [
                                \Carbon\Carbon::now()->startOfWeek(),
                                \Carbon\Carbon::now()->endOfWeek(),
            ...
            if(!empty($employee->team_lead) && !empty($employee->team_id) && $employee->team_lead == '1'){
                ...
                if(\Auth::user()->type=='company'){
                    $deals = Deal::select('deals.*')->join('user_deals', 'user_deals.deal_id', '=', 'deals.id')->where('deals.pipeline_id', '=', $pipeline->id)->distinct()->orderBy('deals.order')->get();
                }
                else{
    ...
    {
        $usr = \Auth::user();
        $empId = Employee::where('user_id', $usr->id)->first();
        ...
        if($usr->can('create deal'))
        {
            ...
            else
            {
                ...
                if($usr->type=='company' || $empId->team_lead == 1){
                    $deal->approval = 'Approved';
                }
                else{
                    $deal->approval = 'Pending';
                }

                $words = explode(' ', $request->name);
                $projectCode = '';
                foreach ($words as $word) {
                    $projectCode .= substr($word, 0, 1);
                }

                $deal->reference = strtoupper($request->company_code). '-'. strtoupper($projectCode) . '/'. now()->format('Y/md') . '-' . rand(10,99);
    ...
    public function show(Deal $deal)
    {
        if($deal->is_active)
        {
            ...
            $permission        = [];
            $customFields      = CustomField::where('module', '=', 'deal')->get();
            $dealDetails      = DealDetail::where('deal_id', '=', $deal->id)->first();
            $deal->customField = CustomField::getData($deal, 'deal')->toArray();

            return view('deals.show', compact('deal', 'dealDetails', 'customFields', 'calenderTasks', 'permission'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission Denied.'));
    ...
    public function edit(Deal $deal)
    {
        if(\Auth::user()->can('edit deal'))
        {
            if($deal->created_by == \Auth::user()->ownerId())
            {
                ...
                return view('deals.edit', compact('deal', 'dealDetails', 'pipelines', 'sources', 'products', 'customFields'));
            }
            else
            {
                return response()->json(['error' => __('Permission Denied.')], 401);
    ...
    public function update(Request $request, Deal $deal)
    {
        if(\Auth::user()->can('edit deal'))
        {
            if($deal->created_by == \Auth::user()->ownerId())
            {
                ...
                DealDetail::where('deal_id', $deal->id)->update([
                    'client_name' => $request->client_name,
                    'email' => $request->email,
                    'phone' => $request->phone,
                    'address' => $request->address,
...
//                ClientPermission::where('deal_id', '=', $deal->id)->delete();
    ...
    public function approveDeal($id){
        ...
        return redirect()->back()->with('success', __('Deal Approved!'));
    ...
    public function rejectDeal($id){
        ...
        return redirect()->back()->with('success', __('Deal Rejected!'));
    }
