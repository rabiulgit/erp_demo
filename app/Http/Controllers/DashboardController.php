     ...
     public function hrm_dashboard_index()
    {
        if (Auth::check()) {
            ...
            if ($user->type != 'client' && $user->type != 'company') {
                ...
                // dd($currentDate);
                $holidays = Holiday::whereDate('date', '<=', $currentDate)
                    ->whereDate('end_date', '>=', $currentDate)
                    ->get();

                $userId = Auth::id();

                $attendance = AttendanceLog::whereDate('date', '=', $currentDate)
                ...
                if ($attendance && $attendance->status === 'Absent') {
                    ...
                    $holiday = ($current_date->isFriday() || $current_date->isSaturday());

                    // Check govt. holiday date
                    $govtHoliday = Holiday::where('date', '<=', $currentDate)
                        ->where('end_date', '>=', $currentDate)
                        ->exists();


                    // Check if user is on leave for this date
                    $onLeave = Leave::where('employee_id', $attendance->employee->id)
                        ->where('start_date', '<=', $currentDate)
                        ->where('end_date', '>=', $currentDate)
                        ->exists();

                    // If holiday or on leave
                    if ($holiday) {
                        $attendance->status = 'Holiday';
                    }

                    else if($govtHoliday){
                        $attendance->status = 'Govt. Holiday';
                    }
                ...
                return view('dashboard.dashboard', compact('attendance', 'holidays', 'arrEvents', 'announcements', 'employees', 'meetings', 'employeeAttendance', 'officeTime'));
            } else {
