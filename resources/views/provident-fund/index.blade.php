...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item">{{ __('Provident Fund') }}</li>
@endsection
...
@section('action-btn')
    <div class="float-end">
        <a href="{{ route('provident.export') }}" data-bs-toggle="tooltip" title="{{ __('Export') }}"
                ...
                <div class="card-body table-border-style">
                    <div class="table-responsive">
                        <table class="table datatable">
                            ...
                            <tbody>
                                @foreach ($providentFunds as $provident)
                                    <tr>
                                        <td>{{ $provident->employee_id }}</td>
                                        <td>{{ $provident->name }}</td>
                                        <td>{{ $provident->from_date }}</td>
                                        <td>{{ $provident->to_date }}</td>
                                        <td>{{ \Auth::user()->priceFormat($provident->total_pf) }}</td>
                                        <td>{{ \Auth::user()->priceFormat($provident->totalPI) }}</td>
                                        <td>{{ \Auth::user()->priceFormat($provident->total_pf + $provident->totalPI) }}</td>
                                        <td>
                                            @if ($provident->total_pf > 0)
                                                <div class="action-btn bg-primary">
                                                    <a href="{{ route('provident-fund.employee', \Illuminate\Support\Facades\Crypt::encrypt($provident->employee_id)) }}"
    ...
    <div id="printableArea" class="mt-2">
        ...
        <div class="invoice-print">
            ...
            <div class="row text-center">
                <h4 class="mb-2">Provident Fund Report</h4>
            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm">
                                <tbody>
                                    ...
                                    @foreach ($providentFunds as $provident)
                                    @php
                                        $total_fund += $provident->total_pf;
                                        $total_interest += $provident->totalPI;
                                    @endphp
                                        <tr>
                                            <td>{{ $provident->employee_id }}</td>
                                            <td>{{ $provident->name }}</td>
