                            ...
                            <div class="card-body kanban-box" id="task-list-{{$stage->id}}" data-id="{{$stage->id}}">
                                @foreach($deals as $deal)
                                    <div class="card" data-id="{{$deal->id}}">
                                        <div class="pt-3 ps-3">
                                            @if ($deal->approval == "Pending")
                                                <div class="status_badge badge bg-warning p-2 px-3 rounded">Pending Approval</div>
                                            @endif
                                            @if ($deal->approval == "Rejected")
                                                ...
                                                @if(Auth::user()->type != 'client')
                                                    <div class="btn-group card-option">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-end">
                                                            ...
                                                            <?php
                                                                $user=\Auth::user();
                                                                $empId = \App\Models\Employee::where('user_id', $user->id)->first();
                                                            ?>
                                                            @if ($user->type=='company' || $empId->team_lead==1)
                                                                @if ($deal->approval=='Pending')
                                                                    <a href="{{route('deal.approve', $deal->id)}}" class="dropdown-item">
                                                                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-check" width="15" height="15" viewBox="0 0 24 24" stroke-width="1.5" stroke="#293240" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                                                            <path d="M5 12l5 5l10 -10" />
                                                                        </svg>
                                                                        <span> {{__('Approve')}} </span>
                                                                    </a>
                                                                    <a href="{{route('deal.reject', $deal->id)}}" class="dropdown-item">
                                                                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-check" width="15" height="15" viewBox="0 0 24 24" stroke-width="1.5" stroke="#293240" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
                                                                            <path d="M18 6l-12 12" />
                                                                            <path d="M6 6l12 12" />
