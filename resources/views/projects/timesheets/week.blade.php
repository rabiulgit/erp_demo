    ...
    <tfooter>
        <tr class="bg-primary text-light">
            ...
            @foreach ($totalDateTimes as $key => $totaldatetime)
                <td class="total-date-time" > <input class="form-control bg-transparent text-light {{ $totaldatetime != '00:00' ? 'border-dark' : 'border-white' }}  wid-120" type="text" value="{{ $totaldatetime != '00:00' ? $totaldatetime : '00:00' }}"> </td>
            @endforeach
            <td class="text-center total-value1">
                <input class="form-control bg-transparent text-light {{ $calculatedtotaltaskdatetime != '00:00' ? 'border-dark' : 'border-white' }} wid-120" type="text" value="{{ $calculatedtotaltaskdatetime != '00:00' ? $calculatedtotaltaskdatetime : '00:00' }}">
            </td>
        </tr>
    </tfooter>
</table>
...
<div class="text-center d-flex align-items-center justify-content-center mt-4 mb-5 timelogged">
    ...
    <span class="p-2  f-w-900 rounded  bg-primary d-inline-block border  text-light border-dark">{{ $calculatedtotaltaskdatetime . __(' Hours') }}</span>
</div>
