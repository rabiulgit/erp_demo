...
{{ Form::open(['url' => 'projects', 'method' => 'post', 'enctype' => 'multipart/form-data', 'id' => 'project_form']) }}
<div class="modal-body">
    ...
    <div class="row">
        ...
        <div class="col-sm-6 col-md-4">
            <div class="form-group">
                ...
                <div class="input-group">
                    {{ Form::number('budget', null, ['class' => 'form-control', 'min' => '0']) }}
                    <div class="input-group-append">
                        <div class="input-group-text" style="border-radius: 0 6px 6px 0;">
                            {{ $settings['site_currency_symbol'] }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-6 col-md-4">
            <div class="form-group">
                {{ Form::label('estimated_man_days', __('Estimated Man-Days'), ['class' => 'form-label']) }}
                {{ Form::text('estimated_man_days', 0, ['class' => 'form-control', 'readonly' => 'readonly', 'id' => 'estimated_man_days']) }}
            </div>
        </div>
...
@php
    $startTime = DateTime::createFromFormat('H:i', $settings['company_start_time']);
    $endTime = DateTime::createFromFormat('H:i', $settings['company_end_time']);

    $diff = $endTime->diff($startTime);
    $workingHoursPerDay = $diff->h + ($diff->i / 60);
@endphp
...
<script>
    document.getElementById('project_form').addEventListener('submit', function(event) {
        ...
        if (new Date(startDate) > new Date(endDate)) {
            ...
            show_toastr('error', 'End date should not be greater than start date.');
        }
    });

    document.getElementById('estimated_hrs').addEventListener('input', function() {
        var estimatedHours = parseFloat(this.value);
        var workingHoursPerDay = {{ $workingHoursPerDay }};
        if (!isNaN(estimatedHours)) {
            var estimatedManDays = estimatedHours / workingHoursPerDay;
            document.getElementById('estimated_man_days').value = estimatedManDays.toFixed(1) + ' days';
        } else {
            document.getElementById('estimated_man_days').value = '0';
        }
    });
