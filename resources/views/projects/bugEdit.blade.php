{{ Form::model($bug, array('route' => array('task.bug.update', $project_id, $bug->id), 'method' => 'POST', 'id' => 'edit_bug')) }}
...
<script>
    document.getElementById('edit_bug').addEventListener('submit', function(event) {
        var startDate = document.getElementById('start_date').value;
        var endDate = document.getElementById('due_date').value;

        if (new Date(startDate) > new Date(endDate)) {
            event.preventDefault();
            show_toastr('error', 'End date should not be greater than start date.','error');
        }
    });
</script>